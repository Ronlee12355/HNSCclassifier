  ...
  tabPanel(
    ...
    fluidPage(
      ...
      column(5,
             wellPanel(
               shinyjs::useShinyjs(),
               radioButtons("idType", "Gene ID type:",
                            choices = c("SYMBOL", "ENSEMBL", "ENTREZID", "REFSEQ"),
                            selected = "SYMBOL", inline = TRUE),
               radioButtons("outputType", "Output type:",
                            choices = c("Class labels" = "class",
                                        "Probabilities" = "prob"),
                            selected = "class", inline = TRUE),
               radioButtons("sep", "File separator",
                            choices = c(Comma = ",", Tab = "\t"),
                            selected = ",", inline = TRUE),
               fileInput("Expr", "Upload your expression matrix",
                         accept = c(".csv", ".txt")),
               uiOutput('mRNA_view'),
               uiOutput("mRNA_msg"),
               HTML('Example input dataset could download <a href="HNSCclassifier_example.csv", target="_blank" download="HNSCclassifier_example.csv">HERE</a>'),
